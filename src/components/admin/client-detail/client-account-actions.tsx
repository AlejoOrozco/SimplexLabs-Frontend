...
export function ClientAccountActions({ user, companyIsInactive = false }: ClientAccountActionsProps): JSX.Element {
  const qc = useQueryClient();
  const deleteUser = useDeleteUser();
  const [confirmOpen, setConfirmOpen] = useState(false);
  const showDeactivate = canDeactivateUser(user, companyIsInactive);
