    ...
    <Dialog open={isOpen} onOpenChange={() => {}}>
      ...
      >
        ...
        <p className="mt-2 text-text-secondary">
          Your account has been temporarily deactivated. This may be due to a billing issue or a policy
          violation.
        </p>
...
export function AccountDeactivatedModalRoot(): JSX.Element {
  ...
  const [detail, setDetail] = useState<AccountDeactivatedDetail | null>(null);

  useEffect(() => {
    const onDeactivated = (event: Event): void => {
      const custom = event as CustomEvent<AccountDeactivatedDetail>;
      setDetail(custom.detail ?? null);
      setOpen(true);
    };
    window.addEventListener(ACCOUNT_DEACTIVATED_EVENT, onDeactivated);
    return () => window.removeEventListener(ACCOUNT_DEACTIVATED_EVENT, onDeactivated);
  }, []);
